...
/*
 ...
 по этому пришлось прибегнуть к использованию магических констант и сочитание в системе rgb в виде (254, 255, 255, 1) означает цвет который 
 является задним фоном спрайта и не должен отрисовываться, также было принято решение устанавливать левый верхний пиксель каждого спрайта с 
 задним фоном этим цветом что и можно увидеть в функции класса Game::DrawPlayer где сравнивается взятый пиксель с пикселем заднего фона по
 координатам (0,0), если они совпадают то первый не отрисовывается.
